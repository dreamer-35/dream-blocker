...
- 分层规则：强关键词/引流域名命中即拦，弱关键词需≥2条；自定义词命中即拦
...
- **显示**：恢复本条
- **拉黑作者**：写入 X 官方黑名单（需已登录；先滚动时间线以便捕获 Bearer，成功后有底部提示）
- **信任此人**：加入白名单
...
| 现象 | 处理 |
|------|------|
| 拉黑无反应 / 一直「拉黑中」 | v1.0.2+ 页面内请求；先滚动时间线抓 Bearer |
| Extension context invalidated | 扩展重载后请 **F5 刷新 x.com**；v1.0.4+ 会提示中文而非英文报错 |
| 保存后未生效 | v1.0.4+ popup 会主动推送设置；失败时提示刷新页面 |
| 误伤 | 点「显示」或白名单；检查自定义关键词是否过短（如「骚」）；控制台 `[xyb]` 看命中原因 |
| 误伤多 | v1.0.5 短词词边界匹配；正则去掉裸 `tg`；自定义词慎用单字 |
| 漏拦 | v1.0.5+ 视口内 IntersectionObserver + 滚动补扫 |
| SearchTimeline 429 | v1.0.6+ 去掉定时全量扫；未命中推文标记已处理 |
| 点「显示」后空白 | v1.0.7 本会话不再自动隐藏该条；红条会正确移除 |
| message channel 报错 | v1.0.7 合并为单次 APPLY_SETTINGS + 回调式 sendMessage |
